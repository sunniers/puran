...
$(function(){
	...
	jQuery.fn.rater	= function(options) {
		...
		var settings = {
			...
			title_format	: null,
			info_format	: null,
			image	: 'resources/images/front/stars.jpg',
			imageAll :'resources/images/front/stars-all.gif',
			defaultTips :true,
			clickTips :true,
			width	: 24,
			height	: 24
	...
	var app={
			...
			getBaseInfo:function(){
				T.post("/s/ceativeResourceTransaction/selectCeativeResourceTransaction",
						{"resourceTransactionId":sessionStorage.getItem('resourceTransactionId')},
						function(res){
							console.log(res);
							
							if(res.retCode="10000"){
								var _obj=res.retObject;
								_obj.buildDate=T.dateFormat(new Date(_obj.buildDate),"yyyy-MM-dd hh:mm:ss");
								...
								$('.comMoney').text("￥"+_obj.transactionMoney+"元");//资源数量
			...
			comment:function(){
				$(document).on('click','.comTjiao',function(){
					if(($('#comPl').val()).length < 5||$('.rater-star-result').text().substr(0, 1)<1){
						$.alert('请评论完整');
					}else{
						T.post('/s/evaluate/insertCreativeResourceEvaluate',
								{"transactionId": sessionStorage.getItem('resourceTransactionId'),
								"evaluateStar": $('.rater-star-result').text().substr(0, 1),
								"evaluateValue":$('#comPl').val()},
